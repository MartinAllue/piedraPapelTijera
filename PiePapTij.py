...
def save_data(filename, row):
    header = ["timestamp", "player1_move", "player2_move", "winner"]
    try:
...
def play_and_collect(filename="datos_rps_teclado.csv"):
    ...
    while True:
        ...
        timestamp = time.time()
        save_data(filename, [timestamp, p1_move, p2_move, winner])
